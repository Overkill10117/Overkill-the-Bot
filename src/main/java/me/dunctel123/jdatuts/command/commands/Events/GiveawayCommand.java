...
public class GiveawayCommand implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        if(ctx.getMessage().getContentRaw().equals(".ghelp"))
        {
            ctx.getChannel().sendMessage("<:tada:294906617378504704> Giveaway help: <:tada:294906617378504704>\n"
                    + "`,ghelp` - this message\n"
        ...
        else if(ctx.getMessage().getContentRaw().startsWith(",create"))
        {
        ...
        else if(ctx.getMessage().getContentRaw().startsWith(".reroll"))
        {
            String id = ctx.getMessage().getContentRaw().substring(7).trim();
            if(!id.matches("\\d{17,22}"))
            {
                ctx.getChannel().sendMessage("Invalid message id").queue();
                return;
            }
            Message m = ctx.getChannel().getHistory().getMessageById(id);
            if(m==null)
            {
                ctx.getChannel().sendMessage("Message not found!").queue();
                return;
            }
            m.getReactions()
                ...
                ctx.getChannel().sendMessage("Congratulations to <@"+uid+">! You won the reroll!").queue();
            });
        }
